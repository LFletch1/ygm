...
int main(int argc, char **argv) {
    ...
    ygm::io::line_parser file_reader(world, {"graphs/government_edges.csv"}); // ","
    int num_of_nodes = 7057;

    // ygm::io::line_parser file_reader(world, {"graphs/.csv"}); // ","
    ...
    // world.cout0() << "Compute Nodes, Average Tree Generation Time (microseconds)" << std::endl;;
    ...
    for (int i = 0; i < trees; i++) { 
        ...
        auto process_edge_lambda = [&b_c, &least_compressions, &c_p, &world, 
            ...
            if (edges_processed == c_p) { 
                // world.cout0("Total Compressions: ", total_compressions);
                if (b_c != 0) {
        ...
        world.cout0("Spanning Tree Size: ", spanning_tree_size);
    ...
    // world.cout0() << world.size() / 24 << "," << duration.count() / trees << std::endl;;

    auto edge_count_lambda = [&world](const auto edge_str, const size_t count){
        world.cout() << "(" << edge_str.first << "," << edge_str.second << ")" << ": " <<  count << std::endl;
    };
